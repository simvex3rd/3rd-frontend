...
const clerkErrorMap: Record<string, string> = {
  ...
  "Identifier is invalid.": "유효하지 않은 이메일입니다.",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.status === "complete") {
        ...
        try {
          const clerkEmail = email; // Use the input value, which is a valid email
      ...
      setError(
        translateClerkError(
          clerkError.errors?.[0]?.longMessage || "오류가 발생했습니다."
        )
      );
    } finally {
      setIsLoading(false);
      ...
      >
        ...
        >
          ...
          <form
            ...
            className="flex w-full flex-col items-start gap-[16px] self-stretch"
          >
            {/* Email Field */}
            <div
          ...
          <div className="flex flex-col gap-[8px] w-full">
            <button
              ...
              className="flex h-[40px] w-full items-center justify-center gap-[8px] rounded-[8px] bg-white px-[16px] hover:bg-neutral-100 disabled:opacity-50 transition-colors"
            >
              {oauthLoading === "google" ? (
            ...
            <button
              ...
              disabled={oauthLoading !== null}
              className="flex h-[40px] w-full items-center justify-center gap-[8px] rounded-[8px] bg-neutral-800 border-[1px] border-neutral-700 px-[16px] hover:bg-neutral-700 disabled:opacity-50 transition-colors"
            >
              {oauthLoading === "github" ? (
