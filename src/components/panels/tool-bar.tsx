...
const toolBarVariants = cva(
  "flex items-center justify-center p-2 border-2 border-[#02EEE1]/20 rounded-[24px] bg-white/10 backdrop-blur-md shadow-[0_0_20px_rgba(2,238,225,0.1)] transition-all",
  {
    variants: {
      variant: {
    ...
    <div className={cn(toolBarVariants({ variant }), className)} {...props}>
      {tools.map((tool, index) => (
        <div key={index} className="group relative">
          ...
          />
          <div className="pointer-events-none absolute left-1/2 -top-12 z-10 hidden -translate-x-1/2 whitespace-nowrap rounded-md bg-[#02EEE1] px-3 py-1.5 text-xs font-bold text-black opacity-0 transition-opacity group-hover:block group-hover:opacity-100 shadow-[0_4px_12px_rgba(2,238,225,0.3)]">
            {tool.label}
          </div>
        </div>
      ))}
